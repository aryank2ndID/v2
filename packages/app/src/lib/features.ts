/**
 * SANDHI - feature extraction, TypeScript. Port of web/lib/dsp/features.ts
 * which is a line-for-line translation of ml/features.py. Runs on the phone,
 * on the raw BLE stream, with no network. If this file and the Python one ever
 * disagree, the model is fed different numbers than it was trained on.
 */
...
export function vagFeatures(vag: { fs: number; mic: ArrayLike<number> }): {
  vag_hf_ratio: number;
...
export interface Session {
  walk: WalkSignal;
  sts: StsSignal;
  vag: { fs: number; mic: ArrayLike<number> };
}
