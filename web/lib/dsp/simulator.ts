...
export class Rng {
  ...
  pick<T>(a: readonly T[]): T { return a[this.int(0, a.length)]; }
}
...
export interface Subject {
  ...
  _sev: number; _sevFunc: number; _sevVag: number; _coupling: number; _otherCause: number;
...
export interface Nuisance { gyro_gain: number; accel_gain: number; noise_gain: number; }

export function synthWalk(rng: Rng, sev: number, nz: Partial<Nuisance> = {}) {
...
export function synthSts(rng: Rng, sev: number, nz: Partial<Nuisance> = {}) {
