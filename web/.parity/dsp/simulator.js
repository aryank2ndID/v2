/**
 * SANDHI — signal simulator, TypeScript port of ml/simulator.py.
 *
 * ===================== READ THIS =====================
 ...
 * from a knee. The UI must always say so — see components/SimBadge.tsx, which
 * is rendered anywhere this module's output reaches the screen.
 * =====================================================
 *
 * Generative parameters follow published ranges for knee-OA gait and
 * vibroarthrography (docs/REFERENCES.md). No patient data is used.
 */
...
export class Rng {
    ...
    poisson(lam) {
        if (lam > 30)
            return Math.max(0, Math.round(this.normal(lam, Math.sqrt(lam))));
...
export const BIO_NOISE = 1.95;
export function sampleSubject(rng) {
    ...
    lin += rng.normal(0, BIO_NOISE);
    const sev = 1 / (1 + Math.exp(-lin));
    const cuts = [0.18, 0.4, 0.62, 0.83];
    let kl = 0;
    while (kl < 4 && sev >= cuts[kl])
        kl++;
    if (rng.next() < 0.1)
        kl = clamp(kl + (rng.next() < 0.5 ? 1 : -1), 0, 4);
    const ageDecl = clamp((age - 55) / 35, 0, 1);
    const bmiMech = clamp((bmi - 27) / 12, 0, 1);
    const otherCause = (rng.next() < 0.12 ? 1 : 0) * rng.next();
    const sevFunc = clamp(0.66 * sev + 0.18 * ageDecl + 0.12 * bmiMech + 0.3 * otherCause + rng.normal(0, 0.17), 0, 1);
    ...
    return {
        ...
        _coupling: coupling, _otherCause: otherCause,
        kl_grade: kl, label: kl >= 2 ? 1 : 0,
    };
}
export function synthWalk(rng, sev, nz = {}) {
    ...
    for (const [t0, dur] of phases) {
        ...
        for (const [idx, amp] of [[i0, impact], [i0 + Math.trunc(len * stanceFrac), impact * 0.45]]) {
            if (idx >= 0 && idx < n - 12) {
                for (let d = 0; d < 12; d++)
                    shankAcc[idx + d] += amp * Math.exp(-d / 2.4) * Math.cos(d * 1.15);
            }
        }
    }
    for (let i = 0; i < n; i++) {
...
/* ------------------------------------------------------- sit-to-stand ---- */
export function synthSts(rng, sev, nz = {}) {
...
/* ---------------------------------------------------------------- VAG ---- */
export function synthVag(rng, sev, coupling = 1) {
...
export function synthSession(rng, subj) {
    const nuisance = {
