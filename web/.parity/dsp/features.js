...
export const FEATURE_CHANNEL = Object.fromEntries(FEATURE_NAMES.map((n, i) => [
...
export function percentile(v, q) {
...
function median(v) { return percentile(v, 50); }
function cvPct(v) {
    if (v.length < 2)
        return 0;
    const m = mean(v);
...
function findPeaks(x, minDist, height) {
    const idx = [];
    let last = -1e9;
    for (let i = 1; i < x.length - 1; i++) {
        if (x[i] > height && x[i] >= x[i - 1] && x[i] > x[i + 1]) {
            ...
            else if (idx.length && x[i] > x[idx[idx.length - 1]]) {
                idx[idx.length - 1] = i;
                last = i;
            }
        }
...
export function gaitFeatures(walk) {
    const fs = walk.fs;
...
export function stsFeatures(sts) {
...
export function bandEnergy(x, fs, lo, hi, nbands = 24) {
    const n = x.length;
    const edges = Array.from({ length: nbands + 1 }, (_, i) => lo + ((hi - lo) * i) / nbands);
    const w = new Float64Array(n);
    for (let i = 0; i < n; i++)
        w[i] = n > 8 ? 0.5 - 0.5 * Math.cos((2 * Math.PI * i) / (n - 1)) : 1;
...
export function vagFeatures(vag) {
    ...
    for (let i = 0; i < nfr; i++) {
        if (e[i] > 6 * floor && armed) {
            bursts++;
            armed = false;
        }
        else if (e[i] < 2.5 * floor)
            armed = true;
    }
...
export function extract(session, intake) {
    const v = vagFeatures(session.vag);
    const { _bands, _centers, ...vagOnly } = v;
    const f = { ...gaitFeatures(session.walk), ...stsFeatures(session.sts), ...vagOnly };
    for (const k of INTAKE_FEATURES)
        f[k] = Number(intake[k]);
    return f;
}
export function toVector(f) {
